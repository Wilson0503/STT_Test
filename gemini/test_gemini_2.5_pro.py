...
import tempfile
import time
from google import genai  # 注意：2026 官方建議使用 google-genai
...
MODEL_NAME = "gemini-2.5-pro" # 你指定的 2.5 Pro 版本
...
def test_gemini_audio():
    ...
    if extension not in SUPPORTED_MEDIA_EXTENSIONS:
        print(
            f"❌ 錯誤: 不支援副檔名 {extension}，請使用 {', '.join(sorted(SUPPORTED_MEDIA_EXTENSIONS))}"
        )
        return
    ...
    try:
        ...
        # Step 2: 輪詢狀態 (state 在新 SDK 為 enum，用 .name 比對)
        ...
        # Step 3: 修改後的純逐字稿指令
        # 我們使用 "Transcribe" 作為關鍵字，並禁止它做額外的摘要
        prompt = """
...
"""
        ...
        if 'audio_file' in locals():
            ...
            print(f"\n(已清理臨時檔案)")
        if 'temp_upload_path' in locals() and temp_upload_path and os.path.exists(temp_upload_path):
            try:
